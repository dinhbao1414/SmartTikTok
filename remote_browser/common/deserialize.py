...
class JSRemoteObj:
    def __init__(self, obj_id: str, target, frame_id: int, isolated_exec_id: Union[int, None]):
        super().__init__()
    ...
    async def __isolated_exec_id__(self) -> int:
        ...
        if not self.___isolated_exec_id__:
            ...
            res = await self.__target__.send_cdp(
            ...
            super().__setattr__("___isolated_exec_id__", res['result']["executionContextId"])
        # noinspection PyUnresolvedReferences
        return self.___isolated_exec_id__

    async def __exec_raw__(self, script: str, *args, await_res: bool = False, serialization: str = None, max_depth: int = None, timeout: float = 2, execution_context_id: str = None,
        ...
        """
        from ..exceptions import JSEvalException
        ...
        # print(args)
        try:
            res = await self.__target__.send_cdp("Runtime.callFunctionOn", args, timeout=timeout)
...
async def parse_deep(deep: dict, target, isolated_exec_id: int, frame_id: int, subtype: str = None,
    ...
    from .webelement import WebElement
    # from ..sync.webelement import WebElement as SyncWebElement
    ...
    if class_name in ['NodeList']:
        ...
        obj = JSRemoteObj(target=target, obj_id=obj_id,
                          isolated_exec_id=isolated_exec_id, frame_id=frame_id)
        if 'nodelist(' in description.lower():
            result_count = int(description.lower().split('nodelist(')[1].split(')')[0])
        else:
            result_count = int(description[-2])
        for idx in range(result_count):
            elems.append(await obj.__exec__("return obj[arguments[0]]", idx, serialization="deep",
                                            execution_context_id=context_id))
        return elems

    if class_name == 'DOMRect':
        obj = JSRemoteObj(target=target, obj_id=obj_id, isolated_exec_id=isolated_exec_id, frame_id=frame_id)
        _data =  await obj.__exec__('''return {
            x: obj.x,
            y: obj.y,
            width: obj.width,
            height: obj.height,
    ...
    # non-json types
        ...
        # if 'shadowRoots' in 
        return WebElement(backend_node_id=_value.get('backendNodeId'), obj_id=obj_id, target=target,
                                    loop=loop,
                                    class_name=class_name, context_id=context_id,
                                    isolated_exec_id=isolated_exec_id, frame_id=frame_id, is_iframe=True if class_name == 'HTMLIFrameElement' else False)
